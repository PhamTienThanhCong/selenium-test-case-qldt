...
class HtmlTestResult(TextTestResult):
    ...
    def get_results_summary(self, tests):
        ...
        errorPercentages     = str(round(((errors/ len(tests)) * 100),2)) + "%"
        failurePercentages   = str(round(((failures/ len(tests)) * 100),2)) + "%"
        skipPercentages      = str(round(((skips/ len(tests)) * 100),2)) + "%"
        successPercentages   = str(round(((successes/ len(tests)) * 100),2)) + "%"
        ...
        results_summary = {
            ...
            "errorPercentage": errorPercentages,
            "failurePercentage": failurePercentages,
            "skipPercentage": skipPercentages,
            "successPercentage": successPercentages
        }
    ...
    def generate_file(self, testRunner, report_name, report):
        ...
        with io.open(path_file, "w", encoding="utf-8") as report_file:
            report_file.write(report)
